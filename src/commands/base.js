...
class Command {
	...
	constructor(client, info) {
		...
		/**
		 * Whether the command can only be run in a flood channel
		 * @type {boolean}
		 */
		this.floodOnly = Boolean(info.floodOnly);
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `Команда \`${this.name}\` может быть использована только Холли.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `Команда \`${this.name}\` требует право "${permissions[missing[0]]}".`;
				}
				return oneLine`
					Команда \`${this.name}\` требует следующие права:
