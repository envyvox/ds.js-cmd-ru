const { escapeMarkdown, RichEmbed } = require('discord.js');
...
class Argument {
	...
	async obtain(msg, value, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.embed(new RichEmbed().setColor(color).setDescription(stripIndents`
			${empty ? this.prompt : valid ? valid : `Вы ввели неверное значение. Напишите ${this.label} еще раз.`}
			${oneLine`
				Напишите \`отменить\` чтобы отменить.
				${wait ? `Автоматическая отмена через ${this.wait} секунд.` : ''}
			`}
		`)));
			...
			while(!valid || typeof valid === 'string') {
				...
				if(value) {
					...
					${valid ? valid : oneLine`
						...
						Напишите ${this.label} еще раз.
					`}
					${oneLine`
						Напишите \`отменить\` чтобы отменить, или \`закончить\` чтобы закончить ввод.
						${wait ? `Автоматическая отмена через ${this.wait} секунд.` : ''}
					`}
				`)));
					...
					${oneLine`
						Напишите \`отменить\` чтобы отменить, или \`закончить\` чтобы закончить ввод.
						${wait ? `Автоматическая отмена через ${this.wait} секунд, если вы не ответите.` : ''}
					`}
				`)));
				}
