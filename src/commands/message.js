...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.embed(new RichEmbed().setColor(config.color).setDescription(`Все команды необходимо писать в <#${config.chatId.flood}>.`));
		}
		
		if(this.command.floodOnly && this.command.guildOnly && this.message.channel.id !== config.chatId.flood && this.member.id !== config.hollyId) {
			return;
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.embed(new RichEmbed().setColor(config.color).setDescription(`Команда \`${this.command.name}\` должна использоваться только в NSFW канале.`));
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			if(typeof hasPermission === 'string') return this.embed(new RichEmbed().setColor(config.color).setDescription(hasPermission));
			else return this.embed(new RichEmbed().setColor(config.color).setDescription(`У вас нет прав для использования команды \`${this.command.name}\`.`));
		}
		...
		if(this.message.channel.type === 'text' && this.command.clientPermissions) {
			...
			if(missing.length > 0) {
				...
				if(missing.length === 1) {
					return this.embed(new RichEmbed().setColor(config.color).setDescription(
						`Мне нужно право "${permissions[missing[0]]}" чтобы запустить команду \`${this.command.name}\`.`
					));
				}
				return this.embed(new RichEmbed().setColor(config.color).setDescription(oneLine`
					Мне нужны следующие права, чтобы запустить команду \`${this.command.name}\`:
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				if(result.prompts.length === 0) {
					...
					return this.embed(new RichEmbed().setColor(config.color).setDescription(err.message));
				}
				return this.embed(new RichEmbed().setColor(config.color).setDescription(`Команда отменена.`));
			}
			args = result.values;
		}
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				return this.embed(new RichEmbed().setColor(config.color).setDescription(stripIndents`
					Упс, произошла ошибка при запуске команды: \`${err.name}: ${err.message}\`
					Вы не должны были этого увидеть, простите.
					Покажите это сообщение ${ownerList || 'the bot owner'}${invite ? ` in this server: ${invite}` : '.'}
				`))
			}
		}
	}
	...
	editResponse(response, { type, content, options }) {
		...
		let prepend = '';
		if(type === 'embed(new RichEmbed().setColor(config.color).setDescription') prepend = `${this.message.author}, `;
